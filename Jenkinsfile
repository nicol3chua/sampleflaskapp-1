node {
    ...
    registryHost = "c1.icp:8500/"
    imageName = "${registryHost}${appName}:${env.BUILD_ID}"
    env.BUILDIMG=imageName
    docker.withRegistry('https://c1.icp:8500/', 'docker'){
    stage "Build"

        def pcImg = docker.build("c1.icp:8500/default/flask-app:${env.BUILD_ID}", "-f Dockerfile.ppc64le .")
